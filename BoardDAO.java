...
public class BoardDAO {
    ...
    //검색
    public ArrayList<BoardDTO> getDBListBySearch(String part, String keyword){//특정한 리스트를 받아서 반환
        ArrayList<BoardDTO> datas = new ArrayList<BoardDTO>();
        String sql ="select * from board WHERE "+ part + " LIKE ? ORDER BY num DESC";
        ...
        try {
            ...
            while(rs.next()) {
                ...
                boardDTO.setFilesize(rs.getLong("filesize"));
                datas.add(boardDTO);//datas에 해당 인스턴스를 담는다.
            }
            rs.close();
        } catch(Exception e) {
            e.printStackTrace();
